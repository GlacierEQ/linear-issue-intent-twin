...
**Repository:** `GlacierEQ/linear-issue-intent-twin`  
**Company lens (independent):** Linear (`linear`)  
**Innovation:** Issue Intent Twin  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Linear affiliation, proprietary access, or production deployment.
...
Twin each issue with acceptance predicates and block merges that violate them.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Linear employment, endorsement, or proprietary systems access.
...
File: `src/issue_intent_twin.py`

- Expand `IssueIntentTwin` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_issue_intent_twin.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_issue_intent_twin.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Linear APIs
